...
【ピン定義】（最新版）
  ...
  // PIN_TM1637_CLK  = 3   // 4桁7セグ CLK（現構成ではコメントアウト）
  // PIN_TM1637_DIO  = 4   // 4桁7セグ DIO（現構成ではコメントアウト）
  PIN_BUZZER      = 8   // パッシブブザー
  PIN_SERVO       = 9   // サーボ SG90 信号（Arduino 5V駆動、GNDはArduinoと共通化）
...
【タイマー（millis()用）】
  ...
  // last7SegRefreshMs     : unsigned long = 0   // 7セグ未使用のため無効
...
【その他のフラグ・カウンター】
  ...
  // SEG_REFRESH_MS         : const unsigned long = 5       // 7セグ未使用のため無効
...
- サーボはArduino 5Vで駆動（外部5Vは使わない）。負荷が大きい場合は動作不安定になることがある。
- D0/D1は未使用(Serial予約)。
- D10-D12はLED専用とし、SPIは未使用前提。
- IR受信モジュールのGRY配線（GND=G, VCC=R, OUT=Y）に注意。
- ブレッドボードの同じ穴に2本挿さないよう、列や行を分けて配線する。
- TM1637配線は現状コメントアウト（未使用の場合は省略可）。
---
【実装チェックリスト（最新版）】
- D0/D1に何も接続していない
- D2, D8, D9, D10, D11, D12 の重複割り当てがない（D3/D4は未使用）
...
3. TM1637は現構成では未使用（コメントアウト）のため接続しない。
...
- D2/D8/D9/D10/D11/D12 の配線先が重複していない（D3/D4は未使用）
