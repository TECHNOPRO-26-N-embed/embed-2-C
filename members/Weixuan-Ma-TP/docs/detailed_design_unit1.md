...
| 2 | `warningMode()` | 警告モード（STATE_WARNING）にする | 約500ms間隔でLEDが切り替わる | `Warning Mode LED` が周期的に表示され、LEDが点滅した | [OK] |
| 3 | `warningMode()` | 警告モード（STATE_WARNING）にする | 約300ms間隔でブザーがON/OFFする | `Warning Mode Buzzer` が周期的に表示され、ブザーが断続動作した | [OK] |
| 4 | `readRemote()` | 同じボタンを短時間に連続で押す | 200ms未満の連続入力は無視される | `Ignored repeat input` が表示され、連続入力が無視された | [OK] |
| 5 | `warningMode()` + `stopAll()` | 警告モード中に停止ボタン（CODE_STOP）を押す | LEDとブザーが停止し待機状態へ戻る | `State: STOP` → `Stop All` が表示され、LEDとブザーが停止した | [OK] |
...
なお、点滅速度変更（`updateBlinkSpeed()`）は未実装のため、今回は対象外とした。警告モード（`warningMode()`）は実装済みで、基本動作を確認した。
