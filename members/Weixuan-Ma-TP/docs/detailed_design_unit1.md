...
|:---|:---|:---|:---|:---|:---|
| 1 | `readRemote()` | LED用ボタンを押す | LED用コードが取得される | 実装後に記入 | [ ] |
| 2 | `readRemote()` | ブザー用ボタンを押す | ブザー用コードが取得される | 実装後に記入 | [ ] |
| 3 | `readRemote()` | 停止ボタンを押す | 停止用コードが取得される | 実装後に記入 | [ ] |
| 4 | `readRemote()` | 何も押さない | 0 が返る | 実装後に記入 | [ ] |
| 5 | `readRemote()` | 同じボタンを短時間に連続で押す | RECEIVE_INTERVAL以内の入力は無視される | 実装後に記入 | [ ] |
...
| No | テスト対象の関数 | 入力・操作 | 期待する結果 | 実際の結果 | 合否 |
|:---|:---|:---|:---|:---|:---|
| 1 | `updateState()` | CODE_LED を渡す | currentState が 1 になる | 実装後に記入 | [ ] |
| 2 | `updateState()` | CODE_BUZZER を渡す | currentState が 2 になる | 実装後に記入 | [ ] |
| 3 | `updateState()` | CODE_STOP を渡す | currentState が 3 になる | 実装後に記入 | [ ] |
| 4 | `updateState()` | 未登録コードを渡す | currentState が変化しない | 実装後に記入 | [ ] |
| 5 | `updateState()` | 0 を渡す | currentState が変化しない | 実装後に記入 | [ ] |
...
| No | テスト対象の関数 | 入力・操作 | 期待する結果 | 実際の結果 | 合否 |
|:---|:---|:---|:---|:---|:---|
| 1 | `controlLed()` | state = 1 にする | LEDが点灯または点滅する | 実装後に記入 | [ ] |
| 2 | `controlLed()` | state = 0 にする | LED制御を行わない | 実装後に記入 | [ ] |
| 3 | `controlBuzzer()` | state = 2 にする | ブザーが鳴る | 実装後に記入 | [ ] |
| 4 | `controlBuzzer()` | state = 0 にする | ブザー制御を行わない | 実装後に記入 | [ ] |
| 5 | `stopAll()` | LED動作中に呼び出す | LEDが消灯する | 実装後に記入 | [ ] |
| 6 | `stopAll()` | ブザー動作中に呼び出す | ブザーが停止する | 実装後に記入 | [ ] |
| 7 | `stopAll()` | LED・ブザー両方動作中に呼び出す | 両方が停止し、待機中に戻る | 実装後に記入 | [ ] |
...
| No | テスト対象の関数 | 入力・操作 | 期待する結果 | 実際の結果 | 合否 |
|:---|:---|:---|:---|:---|:---|
| 1 | `controlLed()` | blinkInterval = 500 に設定する | 約500ms間隔でLEDが切り替わる | 実装後に記入 | [ ] |
| 2 | `controlLed()` | blinkInterval = 200 に設定する | 約200ms間隔でLEDが切り替わる | 実装後に記入 | [ ] |
| 3 | `updateBlinkSpeed()` | CODE_BLINK_SPEED を渡す | blinkInterval が切り替わる | 実装後に記入 | [ ] |
| 4 | `warningMode()` | 警告モードを実行する | LED点滅とブザーが同時に動作する | 実装後に記入 | [ ] |
| 5 | `warningMode()` | 警告モード中に停止ボタンを押す | LEDとブザーが停止する | 実装後に記入 | [ ] |
...
|:---|:---|:---|:---|:---|:---|
| 1 | 未登録信号 | 設計していないボタンを押す | 状態が変化せず、誤動作しない | 実装後に記入 | [ ] |
| 2 | 停止処理 | LED通知中に停止ボタンを押す | LEDが消灯し、待機中に戻る | 実装後に記入 | [ ] |
| 3 | 停止処理 | ブザー通知中に停止ボタンを押す | ブザーが停止し、待機中に戻る | 実装後に記入 | [ ] |
| 4 | delay不使用確認 | LED点滅中に別ボタンを押す | 入力を受け付け、状態が切り替わる | 実装後に記入 | [ ] |
| 5 | 配線確認 | LEDまたはブザーが動作しない | ピン番号・GND・抵抗を確認できる | 実装後に記入 | [ ] |
